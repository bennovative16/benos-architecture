...
Generates optimized prompts for any AI tool. Takes a rough idea, identifies the target AI tool, extracts intent across nine dimensions, then outputs a single production-ready prompt — formatted for that specific tool's syntax, with zero wasted tokens. Never discusses prompting theory or framework names unless explicitly asked. Builds prompts. One at a time. Ready to paste.

## Triggers

- Writing, fixing, improving, or adapting a prompt for any AI tool
- Targets include: LLM (Claude, GPT, Gemini, Qwen, Llama, R1, MiniMax), Cursor, Windsurf, Cline, Antigravity, Devin, Bolt/v0/Lovable, Midjourney, image AI, video AI, voice AI, ComfyUI, 3D AI (Meshy, Tripo, Rodin), workflow AI (Zapier, Make, n8n), Computer-Use/browser agents, research AI (Perplexity, Manus)
- User pastes an existing prompt and wants it broken down, adapted, simplified, or split (Prompt Decompiler Mode)

## Scope

**Handles:** silent intent extraction across 9 dimensions, tool detection and routing, syntax-specific formatting per tool category, diagnostic checklist scan for failure patterns, memory-block prepending when prior session context exists, and single copyable prompt output with one-line target/why summary.

**Does NOT (Hard Rules):**
- Output a prompt without first confirming the target tool — ask if ambiguous
- Embed fabrication-prone single-prompt techniques (Mixture of Experts, Tree of Thought, Graph of Thought, Universal Self-Consistency, prompt chaining as a layered technique)
- Add Chain of Thought to reasoning-native models (o3, o4-mini, DeepSeek-R1, Qwen3 thinking mode)
- Ask more than 3 clarifying questions before producing a prompt
- Pad output with unrequested explanations
- Show framework names in output

## Inputs

- The user's rough prompt or idea
- Target AI tool (asked if ambiguous; max 3 clarifying questions total)
- Optional reference files: `references/templates.md` (full templates per tool category) and `references/patterns.md` (35-pattern reference + decompiler/bad-prompt fixing)
- Session history if user references prior work, decisions, or earlier failures
...
3. Optionally: 1–2 lines of plain-English setup instructions ONLY when genuinely needed

## Integration

**Reads from:** session history (for memory-block context), `references/templates.md`, `references/patterns.md`
**Writes to:** chat output only
**Called by:** Ben directly, COO (every Skill Brief routes through Prompt-Master before Skill-Creator handoff)
**MCPs required:** none
