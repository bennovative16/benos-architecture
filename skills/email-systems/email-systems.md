...
This skill covers the full email infrastructure stack: transactional delivery that reaches inboxes, marketing automation that converts, deliverability hygiene, and the provider/architecture decisions that scale across SIPP and WIC ventures.
...
- Venture context (SIPP or WIC) to determine provider split
- Email type (transactional vs. marketing)
- Current DNS/domain setup if diagnosing deliverability
- Volume estimates for IP warm-up planning
- Existing template or code to audit

## Outputs

- DNS record recommendations (SPF, DKIM, DMARC)
- Provider architecture decision (transactional vs. marketing split)
- Queue and retry patterns for transactional sends
- Bounce-handling implementation
- Template patterns (React Email or HTML/plain-text multipart)
- Audit findings against validation checks

## BenOS Integrations

- **Klaviyo MCP** — use for all marketing email flows (campaigns, flows, list management, template creation). Invoke `klaviyo_*` MCP tools when wiring SIPP transactional confirmation emails or WIC marketing automation.
- **SIPP transactional emails** — wire through a dedicated transactional provider (Postmark or Resend) separate from Klaviyo to protect deliverability. Klaviyo handles the marketing side; transactional sends stay isolated.

## Customization Notes

Taken as-is from Antigravity source (Apache 2.0). Generic references to "your email platform" swapped to "Klaviyo" where the context is marketing email. No structural changes. Code examples are language-agnostic and preserved verbatim.
...
IP addresses and providers to protect transactional deliverability. | Examples: Good: Password resets via Postmark, marketing via Klaviyo | Bad: All emails through one account
