...
Strategic planning skill for BenOS. When Ben has a goal but not a path, this skill builds the path — taking a destination and working backward to the first step. Output is a plan, not a task list. Plans have shape: they show what depends on what, what runs in parallel, where the campaign layer needs to be triggered. Task Manager creates the tasks after Backlog Manager sequences them.

## Triggers

- "Strategy Builder —"
- "build a plan for"
- "how do we make X happen"
- "plan out the path to"
- "I want to [goal] help me think through it"
- "what is the strategy for"
- Any request where the output is an execution plan rather than a task list
- Auto-triggered by Inbox when work type is Strategic or Campaign

## Scope

**Handles:** loading BEN.md and current-quarter objectives, sharpening the goal into a one-sentence measurable objective, decomposing into 2–5 projects with scope/done-when/effort/dependencies/parallelism/campaign trigger, building the structured handoff block for Backlog Manager, sequencing diagram, phased planning when scope spans more than one quarter, and flagging Campaign Designer triggers.

**Does NOT:**
- Create tasks in Linear or Craft (Task Manager owns that)
- Prioritize the backlog (Backlog Manager owns that)
- Design campaigns (Campaign Designer owns that)
...
- Produce vague plans — every project has concrete scope and done-when

## Inputs

- The goal Ben states (or work brief from Inbox flagged Strategic/Campaign)
- BEN.md (always — venture priority order, revenue target, active vs parked ventures)
- BenOS / 02 — Goals & KPIs / current-quarter-objectives (always)
- Venture-specific context from calling BA (if applicable)

## Outputs

Inline plan in chat consisting of:
1. Sharpened objective (one sentence — `By [timeframe], [verifiable outcome]`)
2. Plan overview (project breakdown with scope, done-when, effort, dependencies, campaign triggers)
3. Sequencing diagram (text-based — first / parallel / dependent)
4. **HANDOFF → BACKLOG MANAGER** block with objective, sequenced projects + routing, dependencies, and the single first action this week

## Integration

**Reads from:** BEN.md, current-quarter-objectives, calling BA context
**Writes to:** chat only (no Craft or Linear writes)
**Called by:** Ben directly, Inbox (when work type = Strategic or Campaign)
