# TABA Proposal Package Generator (Phase II)
...
Generate a complete TABA (Technical and Business Assistance) proposal package for a new Catalyzing Concepts client — Phase II / full engagement (default $50k, 12–18 months). All outputs use the Catalyzing Concepts brand palette, logo, and the Push + Pull Commercialization Engine framework. Use the `taba-phase1` skill instead for Phase I ($6,500) engagements.
...
- "build a client package" / "start a client package"
- "TABA engagement for"
- "set up a proposal for"
- Reference to an SBIR/STTR awardee that needs commercialization support
- Always proactively when the user names a new client and has not yet started a document set

## Scope

**Handles:** 3-round client intake, customizing the three template scripts in `scripts/` (proposal, budget, letters) only at the `CLIENT CONFIG` block, running them, validating outputs (XML, paragraph/table/image counts, budget math across WP1–WP4 and Yr1/Yr2 splits), Communication Policy block, Infrastructure Scenarios A and B, agency-specific letter variations (NASA / DOE / NSF / DoD), and copying outputs to the Catalyzing Concepts workspace under the standard `[CLIENT_ABBREV]_TABA_*` filenames.

**Does NOT:**
- Edit anything outside the CLIENT CONFIG block in the template scripts
- Bundle separate platform pass-throughs (GoHighLevel / Framer / tools are folded into deliverable pricing)
- Use the Phase I framing — Phase II is the full commercial engine engagement
...
- Agency: NASA | DOE | NSF | DoD (default NASA)

### Round 2 (Scope)
- Budget (default $50k; ask if $35k or custom)
- Year 1 / Year 2 split (default $28k/$22k or even $25k/$25k)
- Proposal number (or `[INSERT PROPOSAL NUMBER]`)
- Client website URL (for Infrastructure Scenario A)

### Round 3 (Content)
- Three target market segments / ICP groups (name, who, what they need to hear)
- Key conferences or BD channels
- Work package adjustments

If user says "just get started" → approximate from technology description and tag `[PLACEHOLDER]`.

## Outputs

Three files copied to `/sessions/.../mnt/Catalyzing Concepts/`:
1. **`[CLIENT_ABBREV]_TABA_Proposal.docx`** — branded proposal with methodology, Communication Policy, Infrastructure Scenarios A & B, 4 Work Packages, budget
2. **`[CLIENT_ABBREV]_TABA_Budget.xlsx`** — Option A ($35k) and Option B ($50k) with live formulas
3. **`[CLIENT_ABBREV]_TABA_Letters.docx`** — TABA Vendor Quote (LOC) with full line-item budget breakdown + Acknowledgement Letter

Validation expectations:
- Proposal: XML VALID, 300+ paragraphs, 35+ tables, 2+ images (logo + engine diagram)
- Letters: XML VALID, 130+ paragraphs, 10+ tables
- Budget: WP1 + WP2 + WP3 + WP4 = totalBudget; line items sum to each WP

## Integration

**Reads from:** template scripts under `scripts/`, engine diagram (optional), Logo Files
**Writes to:** `/sessions/[SESSION]/mnt/outputs/` and `/sessions/[SESSION]/mnt/Catalyzing Concepts/`
**Called by:** Ben directly when starting a new Phase II client engagement
