# Landing Page Generator — BenOS Skill
...
| **Triggers** | "landing page", "build a page", "waitlist page", "product page", "hero section" |

## Purpose

Generates high-converting Next.js/React landing pages from a product description, with complete TSX components, proven copy frameworks (PAS, AIDA, BAB), SEO optimization, and performance-first patterns. Produces actual conversion copy — not placeholder text.
...
- **Product name** — the venture or feature being promoted
- **Tagline** — one-sentence value proposition
- **Target audience** — who the page is for
- **Key pain point** — the problem being solved
- **Key benefit** — primary outcome for the user
- **Pricing tiers** — free/pro/enterprise or description
- **Design style** — dark-saas | clean-minimal | bold-startup | enterprise
- **Copy framework** — PAS | AIDA | BAB

## Outputs

- Complete TSX components (Hero, Features, Pricing, FAQ, Testimonials, CTA, Footer)
- Tailwind CSS class sets matching the chosen design style
- SEO meta tags (title, description, OG image, structured data)
- Copy variants written to the chosen framework
- Performance-optimized Next.js patterns (LCP < 1s, CLS < 0.1, FID < 100ms)

## BenOS Integrations

Read `.agents/product-marketing-context.md` for the active venture before generating. If this file does not exist, prompt the user to run the product-marketing-context skill first.

**Pairs with:**
- `design-taste-frontend` — apply venture-specific design tokens and visual taste before building the page
- `popup-cro` — add exit-intent or timed popups to pages after generation
- `page-cro` — run a CRO audit on the generated page to surface conversion gaps

## Customization Notes

Taken as-is from Antigravity source. No structural changes. Generic tool references swapped to BenOS stack:
- "your project tool" → Linear
- "your email platform" → Klaviyo
- Brand voice analyzer script reference preserved verbatim from source (path: `marketing-skill/content-production/scripts/brand_voice_analyzer.py`)
