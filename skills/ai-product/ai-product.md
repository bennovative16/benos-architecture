...
Guides the design and implementation of AI-powered product features — covering LLM integration patterns, RAG architecture, structured output, prompt engineering, and cost optimization. Use when building or reviewing AI capabilities in SIPP or Bennovative products.
...
- Feature or integration request involving an LLM or AI capability
- Existing code with LLM calls under review
- Architecture question about RAG, prompt design, streaming, or cost

## Outputs

- Pattern recommendations with TypeScript/code examples
- Sharp-edge warnings with severity levels and fixes
- Collaboration delegation guidance (backend, frontend, QA, DevOps)
- Validation checklists for LLM output, security, and cost monitoring

## BenOS Integrations

- Useful for SIPP AI feature development — apply patterns here when implementing Claude API calls, structured output, or chat features in the SIPP Flutter app
- Pairs with `product-manager-toolkit` for scoping AI features before implementation
- Track AI feature work in Linear; use the AI Feature Development workflow below

## Customization Notes

Taken as-is from Antigravity source. No restructuring applied. Generic project tool references left in place — use Linear for issue tracking in BenOS context.
